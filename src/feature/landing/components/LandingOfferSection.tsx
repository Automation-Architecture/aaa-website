...
/** Two cube variants alternate every 1s, matching the Figma animation */
function AnimatedCubeIcon() {
  return (
    <div className="relative h-full w-full">
      <Image
        src="/images/cube-triple.svg"
        alt="Cube cluster variant A"
        fill
        className="object-contain p-4"
        style={{ animation: "cube-swap 2s step-start infinite" }}
      />
      <Image
        src="/images/landing-offer-cube-top.svg"
        alt="Cube cluster variant B"
        fill
        className="object-contain p-4"
        style={{ animation: "cube-swap 2s step-start infinite reverse" }}
      />
    </div>
  );
}
...
export function LandingOfferSection() {
  return (
    <section className="mx-auto max-w-11/12 border border-brand-gray2 bg-brand-white overflow-hidden">
      ...
      {/* ── Row 1: empty left | lime heading right ── */}
      <div className="grid grid-cols-1 border-b border-brand-gray2 min-[1025px]:grid-cols-2">
        ...
        <div className="flex items-center justify-center bg-brand-lime px-5 py-[4.90625rem]">
          <h2 className="text-center text-[clamp(2rem,4vw,3.25rem)] font-normal uppercase leading-[1.1] tracking-[-0.3125rem] text-brand-heading">
      ...
      <div className="grid grid-cols-1 border-b border-brand-gray2 min-[1025px]:grid-cols-2">
        ...
        <div className="flex flex-col justify-between border-b border-brand-gray2 min-[1025px]:border-b-0 min-[1025px]:border-r min-[1025px]:border-brand-gray2">
          <div className="flex items-center gap-7.5 px-7.5 py-8">
            {/* bigger cube: use cube-triple scaled up */}
            <div className="relative h-[10.3325rem] w-[11.28125rem] shrink-0 overflow-hidden rounded-[0.625rem] border border-brand-gray2 bg-brand-offwhite">
              <Image
                src="/images/cube-triple.svg"
            ...
            <h3 className="text-[clamp(1.5rem,2.5vw,2.0625rem)] font-bold leading-8.75 text-brand-heading">
              Do it alone:
            </h3>
          </div>
          <p className="px-7.5 pb-7.5 text-[1rem] leading-7 text-brand-heading">
            ...
            months iterating before anything runs. Maintain and evolve it as
            the landscape shifts underneath you.
          </p>
        </div>

        {/* Right: Do it with us (black) + animated cube */}
        <div className="flex flex-col justify-between bg-brand-black">
          <div className="flex items-center gap-7.5 px-7.5 py-8">
            <div className="relative h-[10.3325rem] w-[11.28125rem] shrink-0 overflow-hidden rounded-[0.625rem] border border-brand-gray2 bg-brand-offwhite">
              <AnimatedCubeIcon />
            </div>
            <h3 className="text-[clamp(1.5rem,2.5vw,2.75rem)] font-bold leading-8.75 text-brand-white">
              Do it with us:
            </h3>
          </div>
          <div className="flex flex-1 items-stretch">
            ...
            <Link
              ...
              className="flex w-28 shrink-0 items-center justify-center border-l border-t border-brand-teal bg-brand-teal transition-opacity hover:opacity-90"
      ...
      {/* ── Row 3: commitment rows with hover animation ── */}
      <div className="flex flex-col divide-y divide-brand-gray2">
        {commitments.map((item) => (
          ...
          >
            {/* left accent column (desktop only) — also transitions */}
            ...
            <p className="relative z-10 text-[1.375rem] font-bold text-brand-black">
              {item.heading}
            </p>
